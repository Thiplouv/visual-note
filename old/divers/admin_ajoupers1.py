...
def save_mat () :
    global pso_lv1, pso_lv2, pso_spe, nom, nmpn, profil
    ch_lv1 = pso_lv1.get()
    ch_lv2 = pso_lv2.get()
    ch_spe = pso_spe.get()
    ...
    ajout_dico(dico_lv1, nmpn, ch_lv1)
    ajout_dico(dico_lv2, nmpn, ch_lv2)
    ajout_dico(dico_spe, nmpn, ch_spe)
...
def check() :
    global pso_lv1, pso_lv2, pso_spe, nom, nmpn, profil
    nom = [pso_nom.get(),pso_prenom.get(),'ISN_2020']  # passage du formulaire (nom et prénom) en tuple
    nmpn = [pso_nom.get(),pso_prenom.get()]  # passage du formulaire (nom et prénom) en tuple
